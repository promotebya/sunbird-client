...
export default function RemindersScreen() {
  ...
  async function scheduleDate(iso: string) {
    ...
    await Notifications.scheduleNotificationAsync({ content, trigger: date }); // no repeats for Date trigger
  }
  ...
  async function cancelAll() {
    Alert.alert('Cancel all reminders?', 'This removes every scheduled reminder.', [
      { text: 'No' },
      { text: 'Yes, remove all', style: 'destructive', onPress: async () => {
  ...
  return (
    <KeyboardAvoidingView behavior={Platform.select({ ios: 'padding' })} style={[styles.container, { backgroundColor: theme.bg }]}>
      <ScrollView contentContainerStyle={{ padding: 16 }}>
        <Text style={[styles.h1, { color: theme.text }]}>Reminders</Text>
        <Text style={{ color: theme.sub, marginTop: 6 }}>Gentle nudges to keep the romance active ✨</Text>
        ...
        <View style={styles.segment}>
          {(['daily', 'weekly', 'interval', 'date'] as Mode[]).map(m => (
            <TouchableOpacity
              key={m}
              onPress={() => setMode(m)}
              style={[styles.segmentBtn, { borderColor: theme.border, backgroundColor: mode === m ? theme.primary : 'transparent' }]}
            >
              <Text style={[styles.segmentText, { color: mode === m ? '#fff' : theme.text }]}>{m[0].toUpperCase() + m.slice(1)}</Text>
            </TouchableOpacity>
          ))}
        </View>
        ...
        <View style={styles.chips}>
          {quickChips.map(c => (
            <TouchableOpacity key={c.label} onPress={c.action} style={[styles.chip, { backgroundColor: theme.card, borderColor: theme.border }]}>
        ...
        {mode === 'daily' && (
          <View style={[styles.card, { backgroundColor: theme.card, borderColor: theme.border }]}>
            ...
            <View style={styles.row}>
              <TextInput keyboardType="number-pad" style={[styles.small, { color: theme.text, borderColor: theme.border }]} value={hour === '' ? '' : String(hour)} onChangeText={t => setHour(t === '' ? '' : Number(t))} placeholder="hh" placeholderTextColor={theme.sub} maxLength={2}/>
              <Text style={[styles.colon, { color: theme.text }]}>:</Text>
              <TextInput keyboardType="number-pad" style={[styles.small, { color: theme.text, borderColor: theme.border }]} value={minute === '' ? '' : String(minute)} onChangeText={t => setMinute(t === '' ? '' : Number(t))} placeholder="mm" placeholderTextColor={theme.sub} maxLength={2}/>
            </View>
          </View>
        )}
        {mode === 'weekly' && (
          <View style={[styles.card, { backgroundColor: theme.card, borderColor: theme.border }]}>
            ...
            <TextInput keyboardType="number-pad" style={[styles.input, { color: theme.text, borderColor: theme.border }]} value={weekday === '' ? '' : String(weekday)} onChangeText={t => setWeekday(t === '' ? '' : Number(t))} placeholder="1=Mon ... 7=Sun" placeholderTextColor={theme.sub} maxLength={1}/>
        ...
        {mode === 'interval' && (
          <View style={[styles.card, { backgroundColor: theme.card, borderColor: theme.border }]}>
            ...
            <TextInput keyboardType="number-pad" style={[styles.input, { color: theme.text, borderColor: theme.border }]} value={seconds === '' ? '' : String(seconds)} onChangeText={t => setSeconds(t === '' ? '' : Number(t))} placeholder="≥ 60" placeholderTextColor={theme.sub}/>
          </View>
        )}
        {mode === 'date' && (
          <View style={[styles.card, { backgroundColor: theme.card, borderColor: theme.border }]}>
            <Text style={[styles.label, { color: theme.sub }]}>Date & time</Text>
            <TextInput style={[styles.input, { color: theme.text, borderColor: theme.border }]} value={dateIso} onChangeText={setDateIso} placeholder="YYYY-MM-DDThh:mm" placeholderTextColor={theme.sub}/>
          </View>
        )}

        {/* Inline validation */}
        {!valid && !!hint && <Text style={{ color: theme.sub, marginTop: 8 }}>{hint}</Text>}

        {/* Actions */}
        <View style={styles.actions}>
          <TouchableOpacity onPress={onAdd} disabled={!valid} style={[styles.actionBtn, { backgroundColor: valid ? theme.primary : theme.ghost, opacity: valid ? 1 : 0.6 }]}>
        ...
        {loading ? (
          ...
          scheduled.map((s) => {
            ...
            else if (trig?.type === 'calendar') when = `Weekly ${weekdayName(trig.weekday)} ${pad(trig.hour)}:${pad(trig.minute)}`;
...
function pad(n?: number) { if (typeof n !== 'number') return '--'; return n < 10 ? `0${n}` : String(n); }
...
const styles = StyleSheet.create({
  ...
  segmentBtn: { paddingVertical: 8, paddingHorizontal: 12, borderRadius: 12, borderWidth: 1 },
  segmentText: { fontWeight: '700' },
  chips: { flexDirection: 'row', flexWrap: 'wrap', gap: 8, marginTop: 12 },
  chip: { paddingVertical: 8, paddingHorizontal: 12, borderRadius: 12, borderWidth: 1 },
  card: { marginTop: 12, borderWidth: 1, borderRadius: 14, padding: 12 },
  label: { fontWeight: '700', marginBottom: 6 },
  input: { borderWidth: 1, borderRadius: 10, paddingHorizontal: 12, paddingVertical: 10 },
  row: { flexDirection: 'row', alignItems: 'center', gap: 8 },
  small: { width: 80, borderWidth: 1, borderRadius: 10, paddingHorizontal: 10, paddingVertical: 10 },
  colon: { fontSize: 18, fontWeight: '700' },
  actions: { flexDirection: 'row', gap: 10, marginTop: 12 },
  actionBtn: { flex: 1, paddingVertical: 14, alignItems: 'center', borderRadius: 12 },
  actionText: { color: '#fff', fontWeight: '800' },
  cardRow: { marginTop: 10, borderWidth: 1, borderRadius: 14, padding: 12, flexDirection: 'row', alignItems: 'center', gap: 12 },
  cardTitle: { fontWeight: '800', fontSize: 16 },
  smallBtn: { paddingVertical: 8, paddingHorizontal: 12, borderRadius: 10 },
  smallBtnText: { color: '#fff', fontWeight: '800' },
  empty: { marginTop: 10, padding: 16, borderWidth: 1, borderRadius: 14, alignItems: 'center' },
  toastWrap: { position: 'absolute', bottom: 24, left: 0, right: 0, alignItems: 'center' },
  toast: { backgroundColor: '#111', paddingHorizontal: 14, paddingVertical: 10, borderRadius: 999 },
