...
// import { redeemPairCode } from '../utils/partner'; // ← hook up your pairing logic if needed
  ...
  const onBarcodeScanned = useCallback(
    async (code: BarCode) => {
      ...
      try {
        ...
        // TODO: integrate your pairing flow, e.g.:
        // await redeemPairCode(value);

        Alert.alert('Scanned', value, [{ text: 'OK' }]);
      } catch (e: any) {
        ...
        <View style={{ paddingHorizontal: tokens.spacing.md, paddingBottom: tokens.spacing.md }}>
          <Pressable onPress={() => setLast(null)} style={styles.lastChip}>
            <ThemedText variant="caption" color="#92400E">
              Last: {last.type} — {last.data.slice(0, 40)}
              {last.data.length > 40 ? '…' : ''}
            </ThemedText>
          </Pressable>
        </View>
